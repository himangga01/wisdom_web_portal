...
- 로컬 monitor는 verified age backup pair의 hash·freshness, disk free, Caddy/tunnel/control/worker launchd 상태, loopback `/health/ready`, notification failed backlog, publication/IndexNow backlog의 집계 숫자만 확인한다. 상담 테이블이나 request body를 읽지 않는다. unhealthy이면 독립 HMAC으로 등록된 loopback Hermes endpoint에 metadata와 오류 코드만 전달하고 Telegram에 직접 연결하지 않는다.
