...
운영에서는 `com.jihye.portal.retention`이 시작 시와 매시간 apply 배치를 자동 실행한다. 로컬 monitor의 `RETENTION_OVERDUE`는 파기 시각이 지났지만 `purged_at_ms`가 없는 행이 허용치보다 많다는 뜻이다. scheduled job의 최근 exit status와 보호된 로그에서 안전한 오류 코드만 확인하고 아래 수동 절차로 이어간다.
