...
import { rollbackRelease } from "../lib/release.mjs";
...
async function main() {
  ...
  if (!apply) {
    printJson(await rollbackRelease(input, {}));
    return;
  }
  ...
  printJson(await rollbackRelease(input, adapter));
}

main().catch((error) => {
  process.stderr.write(`Rollback failed: ${error.code ?? "UNKNOWN"}\n`);
  process.exitCode = 1;
});
